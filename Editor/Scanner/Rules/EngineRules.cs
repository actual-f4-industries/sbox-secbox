...
// Mirror of s&box engine's Sandbox.Access AccessRules. Anything NOT allowed by
// the game-side whitelist becomes at least a Medium finding when seen in an
// editor assembly (editor code bypasses AccessControl entirely — see
// sbox-docs/docs/editor/editor-project.md:13-15).
//
// Ported from sbox-public/engine/Sandbox.Access/Config/*.cs and Rules/*.cs.
// Re-port whenever the engine's rule files change. The format is "Assembly/MemberPath"
// with `*` wildcards and `!` prefix denoting deny-override.
//
// NOT YET PORTED: Numerics.cs (file too large to ingest in one read). Workaround:
// the assembly whitelist includes "System.Numerics.Vectors", so broad use of
// Vector3/Quaternion/Matrix4x4 won't false-positive at the assembly tier.
public static class EngineRules
{
	public static RuleSet Build()
	{
		var rs = new RuleSet();
		foreach ( var a in Assemblies ) rs.AddAssembly( a );
		rs.AddRules( BaseAccess );
		rs.AddRules( Types );
		rs.AddRules( Reflection );
		rs.AddRules( Async );
		rs.AddRules( Exceptions );
		rs.AddRules( Diagnostics );
		rs.AddRules( CompilerGenerated );
		return rs;
	}
